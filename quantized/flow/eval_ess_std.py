...
for split in splits:
    ...
    # Save ESS
    with open(os.path.join(mcmc_path, 'ess_std{}_{}.txt'.format(split, eval_args.lag if eval_args.lag else '')), 'w') as f:
        f.write('list: {}\n'.format(means.tolist()))
        f.write('mean: {}\n'.format(ess_mean))
        f.write('std: {}\n'.format(ess_std))
    if args.split is None:
...
if args.split is None:
    ...
    with open(os.path.join(mcmc_path, 'ess_tot_{}.txt'.format(eval_args.lag if eval_args.lag else '')), 'w') as f:
        f.write('mean: {}\n'.format(tot_mean))
        f.write('std: {}\n'.format(tot_std))
